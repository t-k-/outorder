...
/* the create vector, NOTE: speculative copy on write storage provided
   for fast recovery during wrong path execute (see () for
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
      ...
      if (!rs->completed)
	{
      ...
      if (RUU[RUU_head].ea_comp && !RUU[RUU_head].red)
	{
...
ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  while (RUU_index != branch_index)
	...
	panic("RUU head and tail broken");
      ...
      if (RUU[RUU_index].ea_comp && !RUU[RUU_index].red)
	{
		 ...
		 int odep_name)			/* output register name */
{
  ...
  if (spec_mode)
  {
	  BITMAP_SET(use_spec_cv, CV_BMAP_SZ, odep_name);
	  cctv = &(spec_create_vector[odep_name]);
  } else
...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
	 ...
	 && RUU_num < RUU_size - 1 && LSQ_num < LSQ_size
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
	    {
	      ...
	      ruu_link_idep(rs_red, /* idep_ready[] index */2, in3);
...
tk_dump(FILE *stream, const char * say)				/* output stream */
{
  ...
  while (num_ruu > 0 || num_lsq > 0)
  {
	  ...
	  head_ruu = (head_ruu + 1) % RUU_size;
	  head_lsq = (head_lsq + 1) % LSQ_size;
	  num_ruu--;
	  num_lsq--;
	  fprintf(stream, "\n");
  }
